...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f5f8] text-slate-800 font-sans pb-28">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {currentUser ? (
                ...
                <Link
                  href="/login"
                  className="bg-white text-[#b32657] hover:bg-slate-100 text-xs font-black px-5 py-2 rounded-full transition shadow-sm flex items-center gap-1.5"
                >
                  Giriş / Kayıt →
                </Link>
              )}
          ...
          <div className="p-4 sm:p-5 text-xs text-slate-600 space-y-2 bg-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 border-t border-slate-100 text-[11px]">
              <p className="flex items-center gap-1.5 font-bold text-slate-800">
                <Phone className="w-3.5 h-3.5 text-[#b32657]" /> 0474 212 10 15
              </p>
              <p className="flex items-center gap-1.5 truncate">
                <MapPin className="w-3.5 h-3.5 text-[#b32657] shrink-0" /> Ortakapı Mah. Gazi Ahmet Muhtar Paşa Cad. No: 95
              </p>
              <p className="flex items-center gap-1.5 text-slate-500">
                <Clock className="w-3.5 h-3.5 text-[#b32657]" /> 25-35 dk • Min. sipariş 100,00 ₺
              </p>
            </div>
          </div>
        </section>
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="font-black text-[#b32657] text-xs">
                      {product.price.toFixed(2)} ₺
                    </span>
                    <button
                      onClick={() => addToCart(product)}
                      className="w-6 h-6 rounded-full bg-[#b32657] text-white flex items-center justify-center hover:bg-[#911d44] transition shadow-sm"
                    >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-black whitespace-nowrap transition-all ${
                selectedCategory === "all"
                  ? "bg-[#b32657] text-white shadow-md shadow-pink-900/20"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-2 mt-2 border-t border-slate-100">
                      <span className="font-black text-[#b32657] text-xs sm:text-sm">
                        {product.price.toFixed(2)} ₺
                      </span>
                      ...
                      {inCart ? (
                        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200">
                          <button
                            ...
                            className="w-5 h-5 rounded bg-white text-[#b32657] font-bold text-xs flex items-center justify-center shadow-sm"
                          >
                          ...
                          <button
                            onClick={() => addToCart(product)}
                            className="w-5 h-5 rounded bg-[#b32657] text-white font-bold text-xs flex items-center justify-center shadow-sm"
                          >
                        ...
                        <button
                          onClick={() => addToCart(product)}
                          className="bg-[#b32657] hover:bg-[#911d44] text-white text-[11px] font-black px-3 py-1.5 rounded-xl transition flex items-center gap-1 shadow-sm"
                        >
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-4 left-4 right-4 max-w-6xl mx-auto bg-[#b32657] text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between z-40 border border-pink-400/30">
          <div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative text-slate-800 border border-slate-100">
            ...
            <h2 className="text-lg font-black text-[#b32657] flex items-center gap-2">
            ...
            <form onSubmit={handleCompleteOrder} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-[#b32657]"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-[#b32657] resize-none"
                />
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-base font-black text-[#b32657]">{cartTotal.toFixed(2)} ₺</p>
                </div>

                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-[#b32657] hover:bg-[#911d44] text-white font-bold text-xs px-5 py-2.5 rounded-xl transition flex items-center gap-1.5 shadow-md"
                >
